...
public class Main {
	...
	public static void dfs(int a, int b, int c) {
		...
		if (checkFullOrEmpty(0, a + amount, b - amount) && !state[a + amount][b - amount][c]) {
			dfs(a + amount, b - amount, c);
		}
		amount = moveAmount(1, b, a); //A->B
		if (checkFullOrEmpty(1, b + amount, a - amount) && !state[a - amount][b + amount][c]) {
			dfs(a - amount, b + amount, c);
		}
		amount = moveAmount(0, a, c); //C->A
		if (checkFullOrEmpty(0, a + amount, c - amount) && !state[a + amount][b][c - amount]) {
			dfs(a + amount, b, c - amount);
		}
		amount = moveAmount(2, c, a); //A->C
		if (checkFullOrEmpty(2, c + amount, a - amount) && !state[a - amount][b][c + amount]) {
			dfs(a - amount, b, c + amount);
		}
		amount = moveAmount(0, b, c); //C->B
		if (checkFullOrEmpty(0, b + amount, c - amount) && !state[a][b + amount][c - amount]) {
			dfs(a, b + amount, c - amount);
		}
		amount = moveAmount(2, c, b); //B->C
		if (checkFullOrEmpty(2, c + amount, b - amount) && !state[a][b - amount][c + amount]) {
			dfs(a, b - amount, c + amount);
		}
	}
	
	public static boolean checkFullOrEmpty(int destination, int destinationAmount, int sourceAmount) {
		return size[destination] == destinationAmount || sourceAmount == 0;
	}
